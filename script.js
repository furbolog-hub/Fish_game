...
function startFishing() { 
    ...
    setTimeout(() => { 
        ...
        if (rand < 0.003) { 
            playSound('unique'); handleUnique(); 
        } else if (rand < legendaryChance) { 
            playSound('legendary'); handleLegendary(); 
        } else if (state.bonuses.mask) { 
            catchFish(true); state.bonuses.mask = false; state.attempts--; 
        } else if (rand < getBonusChance()) { 
            playSound('bonus'); handleBonus(); 
...
function showDiceModal() { 
    ...
    rollBtn.onclick = () => { 
        ...
        btn2.className = 'fish-btn'; btn2.innerText = `Множитель x${(1 + (points * 0.1)).toFixed(1)}`; 
...
function closeDice() { 
    ...
    document.getElementById('action-btn').disabled = false; updateUI(); 
}
...
function handleLegendary() { 
    ...
    if (item === "Чешуя Левиафана") { 
        state.attempts += 5; state.leviathanBonus = 2.0; alert("Легендарная находка: Чешуя Левиафана! (+5 попыток, бонус к весу)"); 
    } else if (item === "Послание в бутылке") { 
        state.hasMessageInBottle = true; alert("Легендарная находка: Послание в бутылке! (Секрет возврата веса)"); 
    } else if (item === "Компас потерянных глубин") { 
        state.hasCompass = true; alert("Легендарная находка: Компас потерянных глубин! (Управление погодой)"); 
...
function handleBonus() { 
    state.bonusCount++; let b = Math.random(); 
    if (b < 0.2) { 
        alert("Катушка!"); logCatch("Бонус: Катушка (+1)", 0, false, 'bonus'); state.attempts++; 
    } else if (b < 0.4) { 
        state.bonuses.fins = true; alert("Ласты! x2 улов"); logCatch("Бонус: Ласты (x2)", 0, false, 'bonus'); 
    } else if (b < 0.6) { 
        state.bonuses.mask = true; alert("Маска!"); showModal(); logCatch("Бонус: Маска", 0, false, 'bonus'); 
    } else if (b < 0.8) { 
        state.bonuses.aquaCount++; alert("Акваланг!"); logCatch("Бонус: Акваланг", 0, false, 'bonus'); 
    } else { 
        state.bonuses.filter = true; alert("Фильтр!"); logCatch("Бонус: Фильтр", 0, false, 'bonus'); 
    } 
}
...
function logCatch(name, weight, isTrash, type, isRemoved = false, bonusWeight = 0, isFromChest = false) { 
    state.catches.push({ name, weight, isTrash, type, isStolen: false, isRemoved: isRemoved, bonusWeight: bonusWeight, isFromChest: isFromChest }); 
}
...
function showModal() { 
    document.getElementById('action-btn').disabled = true; 
    const list = document.getElementById('modal-fish-list'); list.innerHTML = ''; 
    const cancelBtn = document.createElement('button'); cancelBtn.className = 'fish-btn'; cancelBtn.innerText = "Ничего не удалять"; 
    cancelBtn.onclick = () => { 
        document.getElementById('modal').classList.add('hidden'); 
    ...
    state.catches.filter(c => !c.isStolen && !c.isRemoved).forEach((c) => { 
        ...
        btn.onclick = () => { 
            c.isRemoved = true; document.getElementById('modal').classList.add('hidden'); 
            updateUI(); renderHistory(); if (state.attempts === 0) endGame(); else document.getElementById('action-btn').disabled = false; 
        }; 
...
function endGame() { 
    ...
    const now = new Date(); const dateStr = now.toLocaleDateString('ru-RU'); const timeStr = now.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }); 
    document.getElementById('final-result').innerHTML = `<strong>Итог: ${total.toFixed(2)} кг</strong><div style="margin-top:10px;">${achs.join('<br>')}</div><div style="margin-top: 10px; padding: 5px; background: rgba(128, 128, 128, 0.2); border-radius: 5px; color: #fff; font-weight: bold;"> ${dateStr} | ${timeStr}</div>`; 
