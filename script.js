...
document.addEventListener('DOMContentLoaded', () => { 
    ...
    if (weatherIcon) {
        weatherIcon.addEventListener('click', (e) => {
            e.stopPropagation(); // Блокируем всплытие для корректной работы в Telegram
            toggleWeatherHelp();
        });
    }
...
function toggleWeatherHelp() { 
    ...
    if (!el.classList.contains('active')) { 
        ...
        let htmlContent = `<p style="font-size:18px;">${helpText[state.weather]}</p>`; 
        if (state.hasCompass) { 
            htmlContent += `<button class="fish-btn" onclick="changeWeather()" style="background:#2481cc; color:white; border-radius:8px; margin-top:10px;">Сменить погоду</button>`; 
        } 
...
function handleLegendary() { 
    ...
    if (item === "Чешуя Левиафана") { 
        ...
        alert("🐉 Чешуя Левиафана! (+5 попыток, +2кг к каждой рыбе)"); 
    } else if (item === "Послание в бутылке") { 
        state.hasMessageInBottle = true; 
        alert("📜 Послание в бутылке! Теперь краденая рыба вернется."); 
    } else if (item === "Компас потерянных глубин") { 
        state.hasCompass = true; 
        alert("🧭 Компас! Теперь вы можете менять погоду."); 
...
function handleBonus() { 
    ...
    if (b < 0.2) { 
        ...
        logCatch("Бонус: Катушка (+1)", 0, false, 'bonus'); 
        state.attempts++; 
    } else if (b < 0.4) { 
        state.bonuses.fins = true; 
        alert("🤿 Ласты! x2 улов"); 
        logCatch("Бонус: Ласты (x2)", 0, false, 'bonus'); 
    } else if (b < 0.6) { 
        state.bonuses.mask = true; 
        alert("🤿 Маска! Скан глубин..."); 
        showModal(); 
        logCatch("Бонус: Маска", 0, false, 'bonus'); 
    } else if (b < 0.8) { 
        state.bonuses.aquaCount++; 
        alert("🌊 Акваланг! x3 за лучшую рыбу"); 
        logCatch("Бонус: Акваланг", 0, false, 'bonus'); 
    } else { 
        state.bonuses.filter = true; 
        alert("🌿 Фильтр! Защита от мусора."); 
        logCatch("Бонус: Фильтр", 0, false, 'bonus'); 
    } 
} 
...
function triggerDebuff() { 
    ...
    if (debuffText && !state.activeDebuffs.includes(debuffText)) { 
        ...
        logCatch(debuffText, 0, false, 'debuff'); 
    } 
} 
...
function endGame() { 
    ...
    document.getElementById('final-result').innerHTML = ` 
        ...
        <div style="margin-top:10px; color: #ffd700;">${achs.join('<br>')}</div> 
        <div style="margin-top: 10px; font-size: 12px; color: #aaa;">${dateStr} | ${timeStr}</div>`; 
