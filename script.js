...
function renderHandbook() {
    ...
    if (typeof gameHandbook === 'undefined') {
        container.innerHTML = "<p style='color:red'>Ошибка: handbook.js не загружен.</p>";
        return;
    }
...
function toggleWeatherHelp() { 
    ...
    if (!el.classList.contains('active')) { 
        const helpText = { 
            ...
            'calm': '🌊 Штиль: Идеально для бонусов. Опасайтесь Рака!', 
            'storm': '🌪️ Шторм: Много хлама, но дебаффы не работают.', 
            'fog': '🌫️ Туман: Шанс легендарок повышен на 3%!' 
        }; 

        let htmlContent = `<p style="font-size:18px;">${helpText[state.weather] || 'Обычная погода'}</p>`; 
        if (state.hasCompass) { 
            htmlContent += `<button class="fish-btn" onclick="changeWeather()" style="background:#2481cc; color:white;">Сменить погоду</button>`; 
        } 
...
function showModal() { 
    ...
    state.catches.filter(c => !c.isStolen && !c.isRemoved).forEach((c) => { 
        ...
        btn.onclick = () => { c.isRemoved = true; modal.classList.add('hidden'); updateUI(); renderHistory(); if (state.attempts === 0) endGame(); else document.getElementById('action-btn').disabled = false; }; 
...
function endGame() { 
    ...
    const now = new Date(); 
    document.getElementById('final-result').innerHTML = `<strong>Итог: ${total.toFixed(2)} кг</strong><div style="margin-top:10px;">${achs.join('<br>')}</div><div style="margin-top: 10px; padding: 5px; background: rgba(128, 128, 128, 0.2); border-radius: 5px; color: #fff;">${now.toLocaleDateString('ru-RU')} | ${now.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</div>`; 
