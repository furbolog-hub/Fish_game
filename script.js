...
hasCompass: false, 
diceMultiplier: 1,
filtersUsed: 0, // Новая переменная
wasAttacked: false, // Новая переменная
transmutedCount: 0 // Новая переменная
}; 
...
'eclipse': '🌑 Затмение: Шанс артефактов удвоен!',
...
else if (b < 0.4) { state.bonuses.fins = true; alert("Ласты! x2"); logCatch("Бонус: Ласты (x2)", 0, true, 'bonus'); } 
...
state.catches.forEach(c => { 
const li = document.createElement('li'); const icon = icons[c.name] || "🎣"; const weightRank = getWeightIcon(c.weight); 
...
let chestIcon = c.isFromChest ? "📦 " : ""; let bonusStr = c.bonusWeight > 0 ? ` <span style="color:#ff00ff">(+${c.bonusWeight.toFixed(1)}кг)</span>` : ''; 
...
function showModal() { 
document.getElementById('action-btn').disabled = true; const list = document.getElementById('modal-fish-list'); list.innerHTML = ''; 
...
const btn = document.createElement('button'); btn.className = 'fish-btn'; let label = c.weight > 0 ? `${c.name} (${c.weight.toFixed(1)} кг)` : c.name; 
...
if (state.bonuses.aquaCount > 0 && validCatches.length > 0) { total = ((totalBase - maxWeight) + (maxWeight * 3 * state.bonuses.aquaCount)) * state.diceMultiplier; } 
