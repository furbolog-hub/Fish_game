...
function toggleWeatherHelp() { 
    ...
    if (!el.classList.contains('active')) { 
        ...
        if (state.hasCompass) { 
            content += `<button class="fish-btn" onclick="changeWeather()" style="background:#2481cc; color:white;">Сменить погоду</button>`; 
        } 
        document.getElementById('help-text').innerHTML = content; 
...
function handleBonus() { 
    ...
    if (b < 0.2) { 
        ...
        state.bonuses.fins = true; alert("🤿 Ласты!"); logCatch("Бонус: Ласты (x2)", 0, false, 'bonus'); 
    } else if (b < 0.6) { 
